...
One skill sits outside the research pipeline: `/news-digest` builds a daily email on transportation, energy, environment, and industrial organization. A small Python pipeline (`scripts/news_digest/`) collects the last day's items from RSS feeds, Claude selects and summarizes them per section, and the result goes out over SMTP. It is scheduled twice, by a GitHub Actions cron and by a Claude Code Routine, both aimed at 10:00 America/New_York, so the email arrives even when one scheduler is unavailable. Change the topics and sources in `feeds.toml` to point the same machinery at any field.
...
| `/news-digest` | `.claude/skills/news-digest/` | Daily news digest email — RSS collection, Claude summaries, SMTP delivery; scheduled at 10:00 ET by GitHub Actions or a Routine |
