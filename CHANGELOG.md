...
- **`/news-digest` skill** (`.claude/skills/news-digest/`) — builds a daily email on transportation, energy, environment, and industrial organization / antitrust. Collects the last day's items from RSS feeds, has Claude select and summarize them per section with strict grounding in the feed text, and delivers over SMTP, an email connector, or (failing both) the session's final message. Falls back to WebSearch when the sandbox's egress policy blocks feed hosts. `disable-model-invocation: true`, since it sends mail.
- **`scripts/news_digest/`** — the pipeline behind the skill: `news_digest.py` (`collect` / `send` / `run` / `check-feeds`), `feeds.toml` (sources, topics, keywords, limits), offline `unittest` suite with generated fixtures, and a setup README. Optional Claude API ranking + summaries via structured outputs when `ANTHROPIC_API_KEY` is set; keyword ranking otherwise. One dead feed never fails a run; the email footer names feeds that returned nothing.
