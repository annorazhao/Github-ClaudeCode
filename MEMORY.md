...
[LEARN:network] Claude Code on the web environments with the default "trusted network access" policy block arbitrary hosts at the egress proxy (curl, `requests`, and `WebFetch` all get a 403; `WebSearch` still works because it is served by Anthropic). A scheduled Routine that must fetch arbitrary sites or speak SMTP therefore needs a full-network environment or an outside runner; `/news-digest` uses a GitHub Actions cron for the network-dependent path and a WebSearch fallback inside sessions. Discovered 2026-09-15 while building the news digest.
